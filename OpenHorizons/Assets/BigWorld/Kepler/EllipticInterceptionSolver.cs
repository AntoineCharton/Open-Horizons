...
namespace SimpleKeplerOrbits
{
	...
	public class EllipticInterceptionSolver : MonoBehaviour
	{
		...
		private List<DoubleVector3> CalculateVelocityDifference(KeplerOrbitMover a, KeplerOrbitMover b, KeplerOrbitData transitionOrbit, double departureTime, double duration, double eccAnomalyDeparture,
		...
		public void CalculateTransitionState()
		{
			...
			{
				DoubleVector3 startPoint       = GetPositionAtGivenTime(refs.Origin, (StartTimeOffset),                  refs.OriginAttractorsChain);
				DoubleVector3 endPoint         = GetPositionAtGivenTime(refs.Target, (StartTimeOffset + TargetDuration), refs.TargetAttractorsChain);
				...
				var      hyperbola        = new HyperbolaData(new DoubleVector3(startPoint.X, startPoint.Y, startPoint.Z), endPoint, attractorPos);
		...
		/// </remarks>
		public static DoubleVector3 GetPositionAtGivenTime(KeplerOrbitMover target, double time, KeplerOrbitMover[] attractorsChain = null)
		{
			if (target == null)
			{
			...
			if (attractorsChain == null || attractorsChain.Length == 0)
			{
				...
				else
				{
					...
					var result        = new DoubleVector3(pos.x, pos.y, pos.z) + target.orbitData.GetFocalPositionAtEccentricAnomaly(finalEccAnom);
					return result;
				}
			}
			else
		...
		/// </remarks>
		public static TrajectoryData CalcTransitionTrajectory(DoubleVector3 p0,        DoubleVector3 p1, DoubleVector3 f0, HyperbolaData hyperbola, double targetDuration, bool isReverseOrbit, double attrMass, double g,
			double                                                       precision, double   semiMajorAxisUpperLimit)
		{
			...
			while (true && tmp < 1e3)
			{
				...
				int conicShapeAligmentSign = DoubleVector3.Dot(transitionOrbitEllipse.Normal, hyperbola.Normal) >= 0 ? 1 : -1;
